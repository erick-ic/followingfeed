...
- 第一页请求用于观察热点首页缓存；
- 第二页请求绕过首页缓存，用于观察数据库列表查询；
- 默认每组 50 次请求、并发 5；
- 等待 Prometheus 抓取后输出缓存命中率、数据库 SELECT P95 和 HTTP P95。

脚本会把运行时接口返回的实际文章总数写入 `result.json`，以报告中的
`environment.dataset_total` 为准。第二页请求要求至少存在 11 篇公开文章；数据不足时脚本仍能运行，
但该路径不具备有效的列表查询性能代表性。对比优化前后结果时，文章数量和内容规模必须保持一致。

运行期间不要同时手工压测，否则两分钟统计窗口会混入其他流量。可以通过环境变量调整：

```bash
FOLLOWINGFEED_BASELINE_REQUESTS=100 \
...
```bash
make observability-report NAME=before-cache
```

命令会完成只读压测、查询 Prometheus，并在
`docs/performance-records/<时间>-before-cache/` 中保存：
...
请求数、并发数等参数仍可通过相同环境变量调整，例如：

```bash
FOLLOWINGFEED_BASELINE_REQUESTS=1000 \
FOLLOWINGFEED_BASELINE_CONCURRENCY=20 \
make observability-report NAME=before-query
```

为了让对比有效，两次报告必须使用相同机器、数据集、请求数、并发数和统计窗口。
建议同一场景运行三轮，使用中位结果排除本机抖动。
...
不要只比较单次请求耗时。至少重复运行三轮，优先比较 P95，并确认错误数为零。
